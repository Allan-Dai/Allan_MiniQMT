...
_ACCOUNT_CONFIG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "account_config.json")
ACCOUNT_CONFIG_FILE = os.environ.get('ACCOUNT_CONFIG_FILE', _ACCOUNT_CONFIG_FILE)
...
DEFAULT_QMT_PATHS = [
    ...
    r'D:\stock\首创证券QMT交易端\userdata_mini',
...
def _read_raw_account_config():
    ...
    try:
        # 🔧 诊断：检查文件路径
        import os
        abs_path = os.path.abspath(ACCOUNT_CONFIG_FILE)
        print(f"[_read_raw_account_config] 尝试读取: {abs_path}")
        if not os.path.exists(ACCOUNT_CONFIG_FILE):
            print(f"[_read_raw_account_config] 文件不存在，当前工作目录: {os.getcwd()}")
        with open(ACCOUNT_CONFIG_FILE, 'r', encoding='utf-8') as f:
            data = json.load(f)
            print(f"[_read_raw_account_config] 读取成功: account_id={data.get('account_id')}")
            return data
    except FileNotFoundError:
        print(f"读取账户配置文件失败: 文件不存在 {ACCOUNT_CONFIG_FILE}")
...
# 使用脚本所在目录的 stock_pool.json（支持多账号隔离到 data_<id>/）
_STOCK_POOL_DIR = os.path.dirname(os.path.abspath(__file__))
STOCK_POOL_FILE = os.path.join(_STOCK_POOL_DIR, "stock_pool.json") 
...
def load_stock_pool(file_path=STOCK_POOL_FILE):
    ...
    try:
        with open(file_path, 'r') as f:
            result = json.load(f)
        print(f"[STOCK_POOL] DEBUG: cwd='{_cwd}', open('{file_path}') 成功读取 {len(result)} 只: {result}")
        return result
    except FileNotFoundError:
        print(f"[STOCK_POOL] DEBUG: cwd='{_cwd}', open('{file_path}') 文件不存在，回退 DEFAULT_STOCK_POOL")
